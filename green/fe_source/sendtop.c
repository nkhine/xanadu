...
{
/*
 fprintf(stderr,"sendinsert  docid  ");
puttumbler(stderr,docidptr);
 fprintf(stderr,"  vsa  ");
putsmalltumbler(stderr,vsaptr);
*/
...
/*inittask (taskptr, stdin, stdout);
gotoxy (0,screenheight-1);
 fprintf (stderr, "MAKE LINK  ");
puttumbler(stderr,docidptr);
 fprintf(stderr, "  \n");
*/
        sendrequestbuffer (taskptr);
...
#ifdef COMMENT
  void
sendfindnumoflinksfromtothree (taskptr, linkrestrictionptr)
  tasktype *taskptr;
  linkrestrictiontype *linkrestrictionptr;
{
        sendnumber (taskptr->outp,(tdigit) FINDNUMOFLINKSFROMTO);
...
/*
  void
sendfindnextnlinksfromtothree (taskptr, linkrestrictionptr, linkptr, n)
  tasktype *taskptr;
  linkrestrictiontype *linkrestrictionptr;
  tumbler *linkptr;
  nat n;
{
        sendnumber (taskptr->outp, (tdigit)FINDNEXTNLINKSFROMTOTHREE);
        sendlinkrestriction (taskptr->outp, linkrestrictionptr);
        sendtumbler (taskptr->outp, linkptr);
        sendnumber (taskptr->outp, (tdigit)n);
        sendrequestbuffer (taskptr);
}
*/
  ...
  tumbler *docidptr;
{
  ...
  void
sendsetdebug (taskptr, n)
  tasktype *taskptr;
  nat n;
{
        sendnumber (taskptr->outp,(tdigit) SETDEBUG);
        sendnumber (taskptr->outp, (tdigit)n);
        sendrequestbuffer (taskptr);
}
#endif COMMENT
