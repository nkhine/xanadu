...
*/
        ...
        if (!charspantofix) {
                if(/*currentwindow->newlycreateddocumentp*/TRUE) {
                        charspantofix = newcharspaninvm(specptr);
                        inserttextincharspanindocument (charspantofix, specptr->specspanptr, string, nchars);
                        return;
                } else {
  ...
  long numberofnewcharacters;
{
  ...
  void
fixendsetvm (specptr, nchars, fromspecset, tospecset)
  spectype *specptr;
  long nchars;
  spectype *fromspecset,*tospecset;

{
  spectype *localspecptr;
        ...
        adjustendsetlist (localspecptr, fromspecset);
        adjustendsetlist (localspecptr, tospecset);
...
*/
  ...
  void
fixvmforrearrange(docid,fromspecset,tospecset,cutseqptr)
  tumbler *docid;
  spectype *fromspecset,*tospecset;
...
fprintf(stderr,"%d\n",cutseqptr->numberofcuts);
	specptr = specspanalloc();
	movesmalltumbler(docid,&specptr->docid);
	specptr->specspanptr->vstartaddress.smallmantissa[1] = cutseqptr->cutsarray[0].smallmantissa[1];
	temp = specptr->specspanptr-> sizeofspan = cutseqptr->cutsarray[1].smallmantissa[1]-cutseqptr->cutsarray[0].smallmantissa[1];
	
	insertspecptr = specspanalloc();
	movesmalltumbler(docid,&insertspecptr->docid);
	insertspecptr->specspanptr->vstartaddress.smallmantissa[1] = cutseqptr->cutsarray[2].smallmantissa[1];
	...
	fixendsetvm(insertspecptr,temp,fromspecset,tospecset);
	specfree(insertspecptr);
	fixvm(specptr,(char *)NULL,-temp);      
	fixendsetvm(specptr,-temp,fromspecset,tospecset);
	specfree(specptr);
	ffree(textbuffer);
}     
